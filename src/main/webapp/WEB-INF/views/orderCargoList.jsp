...
<ol class="breadcrumb">
 ...
 <li>Kargodaki Siparişler Listesi</li>
</ol>
<%-- <c:out value="${pageContext.request.session}"/> --%>
<form method="post" action="${pageContext.request.contextPath}/admin/searchCargoOrders">
...
<td><button type="button" class="btn buttonProducts openBtn_${i.getSiparisID()}">Ürünleri Gör</button></td>
<td><button type="button" class="btn buttonProducts updateBtn_${i.getSiparisID()}" ">Durum Güncelle</button></td>
<%-- <td><a href="${pageContext.request.contextPath}/admin/orderProducts?SiparisID=${i.getSiparisID()}">Ürünler</a></td> --%>
</tr>
...
<script>
$('.openBtn_${i.getSiparisID()}').on('click',function(){
    $('.products').load('${pageContext.request.contextPath}/admin/orderProductsInCargo?SiparisID=${i.getSiparisID()}',function(){
...
$('.updateBtn_${i.getSiparisID()}').on('click',function(){
...
<div class="modal fade" id="UpdateModal_${i.getSiparisID()}" tabindex="-1" role="dialog" aria-labelledby="UpdateModal" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
     	<form class="form-horizontal" name="orderModel" method="Post" action="${pageContext.request.contextPath}/admin/updateStateCargo/${i.getSiparisID()}?${_csrf.parameterName}=${_csrf.token}" enctype="multipart/form-data">
      			<div class="modal-header">
			...
			<select name="SiparisDurumId" id="SiparisDurumId" class="custom categorySelect selectBox" autocomplete="off" facet-event="true" facet-force="true"">
	 			...
	 			<c:forEach var="i" items="${stateList}">
	 				<c:if test="${ i.getSiparisDurumKod() != 'Hazırlanıyor' and  i.getSiparisDurumKod() != 'Kargoda'}">
    ...
    <ul class="pagination">
        <c:if test="${currentPage != 1}">
            <li class="page-item"><a class="page-link" 
                href="${pageContext.request.contextPath}/admin/kargodakiSiparisler?pageId=${currentPage-1}">
        ...
        <c:forEach begin="1" end="${noOfPages}" var="i">
            <c:choose>
                ...
                <c:otherwise>
                    <li class="page-item"><a class="page-link" 
                        href="${pageContext.request.contextPath}/admin/kargodakiSiparisler?pageId=${i}">${i}</a>
                    </li>
                </c:otherwise>
            </c:choose>
        </c:forEach>
