...
<form method="post" action="${pageContext.request.contextPath}/admin/searchOrder">
...
$('.openBtn_${i.getKullaniciId()}').on('click',function(){
    $('.products').load('${pageContext.request.contextPath}/admin/orderProducts?SiparisID=${i.getSiparisID()}',function(){
     	...
     	<form class="form-horizontal" name="orderModel" method="Post" action="${pageContext.request.contextPath}/admin/updateState/${i.getSiparisID()}?${_csrf.parameterName}=${_csrf.token}" enctype="multipart/form-data">
			...
			<select name="SiparisDurumId" id="SiparisDurumId" class="custom categorySelect selectBox" autocomplete="off" facet-event="true" facet-force="true"">
	 			...
	 			<c:forEach var="i" items="${stateList}">
	 				<c:if test="${ i.getSiparisDurumKod() != 'Hazırlanıyor' and  i.getSiparisDurumKod() != 'TeslimEdildi'}">
    ...
    <ul class="pagination">
        <c:if test="${currentPage != 1}">
            <li class="page-item"><a class="page-link" 
                href="${pageContext.request.contextPath}/admin/searchOrder?searchVal=${searchVal}&pageId=${currentPage-1}">
        ...
        <c:forEach begin="1" end="${noOfPages}" var="i">
            <c:choose>
                ...
                <c:otherwise>
                    <li class="page-item"><a class="page-link" 
                        href="${pageContext.request.contextPath}/admin/searchOrder?searchVal=${searchVal}&pageId=${i}">${i}</a>
                    </li>
                </c:otherwise>
            </c:choose>
        </c:forEach>
